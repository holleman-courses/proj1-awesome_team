...
#define model
def mobilenet_v1(input_shape=[96, 96, 3], num_classes=2, num_filters = 16):
    ...
    x = DepthwiseConv2D((3, 3), strides=(1, 1), padding='same', depth_multiplier=1)(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    ...
    x = DepthwiseConv2D((3, 3), strides=(2, 2), padding='same', depth_multiplier=1)(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
